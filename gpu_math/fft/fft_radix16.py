...
class Radix16FFT:
    ...
    def fft(self, data, inverse=False, gsg=None):
        ...
        for s in range(log16n):
            ...
            num_workgroups_x = n // 1024
            num_workgroups_y = 1
            # print(num_workgroups_x, L, n)
            self.base.graphics_engine.dispatch_compute(
                (num_workgroups_x * 2, 1, 1), 
    ...
    def precompute_twiddles(n):
        # We only need the roots for the current stage's L
        # Or more efficiently, a global table for the max N
        indices = np.arange(n)
        angles = -2.0 * np.pi * indices / n
        twiddles = np.stack([np.cos(angles), np.sin(angles)], axis=-1).astype(np.float32)
        return twiddles # Push this to a ShaderBuffer
...
class FFT16Demo(ShowBase):
    ...
    def run_test(self, N=65536, output=True, test_inverse=True, gsg=None):
        ...
        g_res_handle = self.hmath.fft(x, gsg=gsg)
...
def benchmark_gpu_avg(count = 1000):
    for k, v in {
        "window-type":          "none",
    ...
    for i in range(count):
        t_gpu, t_cpu = app.run_test(N=16**5, output=False, test_inverse=False, gsg=gsg)
